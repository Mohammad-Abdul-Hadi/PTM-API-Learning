...
if __name__ == '__main__':
    ...
    codet5_base = os.path.abspath(os.path.join(root_folder, 'codet5-base'))
    ...
    tokenizer = RobertaTokenizerFast.from_pretrained(codet5_base,  max_len=MAX_LEN)
    ...
    ## Define the CodeT5 Encoder-Decoder model ##

    # set encoder decoder tying to True
    codet5_shared = EncoderDecoderModel.from_encoder_decoder_pretrained(codet5_base, codet5_base, tie_encoder_decoder=True)

    # Show the vocab size to check it has been loaded
    print('Vocab Size: ', codet5_shared.config.encoder.vocab_size)

    # set special tokens
    codet5_shared.config.decoder_start_token_id = tokenizer.bos_token_id                                             
    codet5_shared.config.eos_token_id = tokenizer.eos_token_id
    codet5_shared.config.pad_token_id = tokenizer.pad_token_id

    # sensible parameters for beam search
    # set decoding params                               
    codet5_shared.config.max_length = SUMMARY_LEN
    codet5_shared.config.early_stopping = True
    codet5_shared.config.no_repeat_ngram_size = 1
    codet5_shared.config.length_penalty = 2.0
    codet5_shared.config.repetition_penalty = 3.0
    codet5_shared.config.num_beams = 10
    codet5_shared.config.vocab_size = codet5_shared.config.encoder.vocab_size
