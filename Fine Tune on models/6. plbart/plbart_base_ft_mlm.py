...
if __name__ == '__main__':
    ...
    plbart_base = os.path.abspath(os.path.join(root_folder, 'plbart-base'))
    ...
    tokenizer = RobertaTokenizerFast.from_pretrained(plbart_base,  max_len=MAX_LEN)
    ...
    ## Define the plbart Encoder-Decoder model ##

    # set encoder decoder tying to True
    plbart_shared = EncoderDecoderModel.from_encoder_decoder_pretrained(plbart_base, plbart_base, tie_encoder_decoder=True)

    # Show the vocab size to check it has been loaded
    print('Vocab Size: ', plbart_shared.config.encoder.vocab_size)

    # set special tokens
    plbart_shared.config.decoder_start_token_id = tokenizer.bos_token_id                                             
    plbart_shared.config.eos_token_id = tokenizer.eos_token_id
    plbart_shared.config.pad_token_id = tokenizer.pad_token_id

    # sensible parameters for beam search
    # set decoding params                               
    plbart_shared.config.max_length = SUMMARY_LEN
    plbart_shared.config.early_stopping = True
    plbart_shared.config.no_repeat_ngram_size = 1
    plbart_shared.config.length_penalty = 2.0
    plbart_shared.config.repetition_penalty = 3.0
    plbart_shared.config.num_beams = 10
    plbart_shared.config.vocab_size = plbart_shared.config.encoder.vocab_size
